    ...
    if(sqlite3_step(stmt) == SQLITE_ROW)
    {
...

      {
        ...
        while(sqlite3_step(stmt) == SQLITE_ROW)
          rowids = g_list_append(rowids, (void *)(long long)sqlite3_column_int(stmt, 0));
        sqlite3_finalize(stmt);
        ...
        while(r)
        {
          snprintf(query, sizeof(query), "UPDATE memory.history SET num=%d WHERE rowid=%lld", v, (long long)(r->data));
