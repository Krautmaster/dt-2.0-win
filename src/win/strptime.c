...
#define get_number(from, to, n, p, v) \
	do {									      \
		int __n = n;							      \
		v = 0;								      \
		while (*p == ' ')							      \
		...
		do {								      \
			v *= 10;							      \
			v += *p++ - '0';						      \
		} while (--__n > 0 && v * 10 <= to && *p >= '0' && *p <= '9');	      \
		if (v < from || v > to)						      \
		  return NULL;							      \
	} while (0)

/* We don't have the alternate representation.  */				
# define get_alt_number(from, to, n, p, v)  get_number(from, to, n, p, v)
...
enum locale_status *decided)
{
	...
	while (*fmt != '\0')
	{
		...
		/* Any character but `%' must be matched by the same character in the iput string.  */
		if (*fmt != '%')
		{
			match_char (*fmt++, *rp++);
			continue;
		...
		case 'x':
			{
				...
				loca = GetThreadLocale();
				GetLocaleInfo(loca, LOCALE_SSHORTDATE, winpic, 100);
		...
		case 'H':
			...
			get_number (0, 23, 2, rp, val);
			tm->tm_hour = val;
			have_I = 0;
			break;
		case 'I':
			/* Match hour in 12-hour clock.  */
			get_number (1, 12, 2, rp, val);
			tm->tm_hour = val % 12;
			have_I = 1;
			break;
		case 'j':
			/* Match day number of year.  */
			get_number (1, 366, 3, rp, val);
			tm->tm_yday = val - 1;
			have_yday = 1;
			break;
		case 'm':
		...
		case 's':
			{
				...
				doing this.  */
				time_t secs = 0;
				if (*rp < '0' || *rp > '9') /* We need at least one digit.  */
				return NULL;
		...
		case 'X':
			{
				...
				loca = GetThreadLocale();
				GetLocaleInfo(loca, LOCALE_STIMEFORMAT, winpic, 100);
		...
		case 'V':
		case 'W':
			get_number (0, 53, 2, rp, val);
			/* XXX This cannot determine any field in TM without some
				information.  */
			break;
		case 'w':
			/* Match number of weekday.  */
			get_number (0, 6, 1, rp, val);
			tm->tm_wday = val;
			have_wday = 1;
			break;
		case 'y':
		...
		case 'O':
			...
			case 'M':
				...
				get_alt_number (0, 59, 2, rp, val);
				tm->tm_min = val;
				break;
			case 'S':
				/* Match seconds using alternate numeric symbols.  */
				get_alt_number (0, 61, 2, rp, val);
				tm->tm_sec = val;
				break;
			case 'U':
			case 'V':
			case 'W':
				get_alt_number (0, 53, 2, rp, val);
				/* XXX This cannot determine any field in TM without
								further information.  */
				break;
			case 'w':
				/* Match number of weekday using alternate numeric symbols.  */
				get_alt_number (0, 6, 1, rp, val);
				tm->tm_wday = val;
				have_wday = 1;
				break;
			case 'y':
				/* Match year within century using alternate numeric symbols.  */
				get_alt_number (0, 99, 2, rp, val);
				tm->tm_year = val >= 69 ? val : val + 100;
				want_xday = 1;
				break;
			default:
...
void get_locale_strings(void)
{
	...
	tm.tm_hour = 1;
	/* in locales where these are unused, they may be empty: better not to reset them then */
